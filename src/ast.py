...
class WhereConditionParser(ASTreeNodeParser):
	...
	def convertItemListToExpList(self, where_condition):
		...
		if len(tmp_list) == 1:
			...
			# cases like ( COND_AND )
			if len(tmp_list) == 3:
				if tmp_list[0].token_name == "LPAREN" and tmp_list[2].token_name == "RPAREN" 
					and (tmp_list[1].token_name == "COND_AND" or tmp_list[1].token_name == "COND_OR"):
