...
import org.apache.hadoop.mapreduce.lib.partition.*;
	...
	public static class Map extends  Mapper<Object, Text, IntWritable, Text>{
		...
		public void setup(Context context) throws IOException, InterruptedException {
			...
			if (filename.compareTo("testquery4") == 0){
				filetag = 2;
			}
		}
		...
		public void map(Object key, Text value, Context context) throws IOException, InterruptedException{
			...
			if (filetag == 1){
				...
				if (line_buf[8].compareTo("R") == 0){
						...
						context.write(new IntWritable(Integer.parseInt(line_buf[0])), new Text(1+"|"+dispatch.toString()+"|"+Double.parseDouble(line_buf[5])+Double.parseDouble(line_buf[6])+Integer.parseInt(line_buf[0])));
				}
			}

			if (filetag == 2){

				context.write(new IntWritable(Integer.parseInt(line_buf[7])), new Text(2+"||"+Integer.parseInt(line_buf[0])+line_buf[1]+Double.parseDouble(line_buf[2])+line_buf[3]+line_buf[4]+line_buf[5]+Integer.parseInt(line_buf[6])+Integer.parseInt(line_buf[7])));
			}
		...
		public void reduce(IntWritable key, Iterable<Text> v, Context context) throws IOExceptiuon, InterruptedException {
			...
			for (int i = 0; i < al_left_0.size(); i++) {
				...
				for (int j = 0; j < al_right_0.size(); j++) {
					...
					tmp_output[0].add(Integer.parseInt(left_buf_0[0])+left_buf_0[1]+Double.parseDouble(left_buf_0[2])+left_buf_0[3]+left_buf_0[4]+left_buf_0[5]+Double.parseDouble(right_buf_0[0])+Double.parseDouble(right_buf_0[1])+Integer.parseInt(left_buf_0[6]));
				}
			}
	...
	public int run(String[] args) throws Exception {
		...
		Job job = new Job(conf, "testquery3");
		job.setJarByClass(testquery3.class);
...
setMapperClass(Map.class);
	...
	public static void main(String[] args) throws Exception {
		...
		int res = ToolRunner.run(new Configuration(), new testquery3(), args);
		System.exit(res);
	}
