...
def isColumnUniqueInTable(column_name, table_name):
	...
	tmp_list = filter(lambda x:x == column_name, 
					global_table_dic[table_name].column_list)
	return (len(tmp_list) == 1)
	
def isColumnInTable(column_name, table_name):
	return (table_name in global_table_dic.keys()) and 
			(column_name in [x.column_name for x in global_table_dic[table_name].column_list])
