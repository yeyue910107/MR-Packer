...
def genXMLTree(queryFile, tmpFilePath):
    ...
    subprocess.check_call(EXEC_DIR + '/MRPackerFront.exe ' +  queryFile + ' > ' + tmpFilePath, shell=True)

def genHadoopJobs(schemaFile, tmpFilePath, queryName, queryInputPath, queryOutputPath):
    os.chdir(CURRENT_DIR)
    cmd = 'python run.py ' + schemaFile + ' ' + tmpFilePath + ' ' + queryName + ' ' + queryInputPath + ' ' + queryOutputPath
    print cmd
...
def main():
    ...
    if (len(sys.argv) == 3):
        queryName = "testquery"
        queryInputPath = "MRPackerInput/"
        queryOutputPath = "MRPackerOutput/"
    else:
